...
public class SelectionView: DefaultView {
  ...
  public var didSelect: (index: NSIndexPath, selected: SelectOption?) -> () = {_ in } { didSet { vc.didSelect = didSelect } }
//  public var didSelect: (selected: SelectOption) -> () = {_ in } { didSet { vc.didSelect = didSelect } }
  ...
  public var expended = false { didSet {
    if expended { self.superview!.bringSubviewToFront(self) } } }
  ...
  public override func layoutSubviews() {
    ...
    animate {
      if !self.expended {
        ...
        let h = CGFloat([self.collectionData.count * Int(self.vc.cellHeight), Int(screenHeight() / 2)].minElement()!)
